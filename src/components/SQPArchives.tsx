        ...
        {filteredQuestions.map(q => (
          <div key={q.id} className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm hover:shadow-md transition-shadow relative">
            ...
            {expandedQuestions.has(q.id) ? (
              ...
              <button
                onClick={() => toggleExpand(q.id)}
                className="w-full flex items-center justify-between gap-4 p-4 bg-gradient-to-r from-indigo-50 to-amber-50 hover:from-indigo-100 hover:to-amber-100 rounded-2xl border border-dashed border-indigo-200 transition-all group"
              >
